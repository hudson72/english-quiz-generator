...
export class QuizzesService {
  ...
  async add(newQuiz: AddNewQuizDto, user: Users): Promise<AddNewQuizResponse> {
    ...
    if (await Quizzes.findOne({ where: { quizName } }))
      throw new HttpException(
        `Sorry, quizName: '${quizName}' already exists! Please provide a new quizName.`,
        400,
      );
  ...
  async delete(id: number): Promise<DeleteQuizResponse> {
    if (await Quizzes.findOne({ where: { id } })) {
      ...
      throw new HttpException(
        `Sorry, quiz with ID: '${id}' doesn't exist!`,
        404,
      );
    }
  }
  async update(
    ...
    if (!quizName && !totalQuestions)
      throw new HttpException(
        `Sorry, incorrect or missing data for update! Please provide a new value for at least one of the following keys: 'quizName', 'totalQuestions'.`,
        400,
      );
    if (totalQuestions === quizToUpdate.totalQuestions)
      throw new HttpException(
        `Sorry, the given 'totalQuestions' and the existing one are the same: '${totalQuestions}'!`,
        400,
      );
    if (quizName === quizToUpdate.quizName)
      throw new HttpException(
        `Sorry, the given 'quizName' and the existing one are the same: '${quizName}'!`,
        400,
      );
